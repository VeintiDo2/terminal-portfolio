    ...
    return (
        <div className={`${closing ? "animate-disappear" : "animate-appear"} fixed inset-0 z-50 flex items-center justify-center bg-black/25`} >
            <div className="bg-terminalGreenDark border border-terminalGreen shadow-glow min-w-100 min-h-50 text-terminalGreen">
                <TerminalButton icon="Close" size="small" onClick={closeModal} soundActivated={soundActivated} sound={{ playClick: () => playErrorClick(), playHover: () => playHover() }} />
                <article className="flex flex-col gap-1 p-2">

                    <TerminalButton label="Español" variant="console" size="small" soundActivated={soundActivated} sound={{ playClick: () => playClick(), playHover: () => playHover() }} />
                    <TerminalButton label="English" variant="console" size="small" soundActivated={soundActivated} sound={{ playClick: () => playClick(), playHover: () => playHover() }} />
